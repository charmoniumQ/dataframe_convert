# Command-Line Help for `dataframe_convert`

This document contains the help content for the `dataframe_convert` command-line program.

**Command Overview:**

* [`dataframe_convert`↴](#dataframe_convert)
* [`dataframe_convert cat`↴](#dataframe_convert-cat)
* [`dataframe_convert metadata`↴](#dataframe_convert-metadata)
...
* `<PATHS>` — N>0 input paths followed by 1 output path

###### **Options:**

* `-o`, `--output-format <OUTPUT_FORMAT>` — output_format will be inferred if not given.

   Supports: CSV/TSV, Parquet, JSON, IPC (arrow), XLSX, SQLite, DuckDB, MD/Markdown,

  Default value: ``
* `-i`, `--input-format <INPUT_FORMAT>` — input_format will be inferred if not given.
  ...
  Default value: ``
* `--dtypes <COLUMN>` — Specification like: `col_name=type_name`
...
* `-N`, `--no-infer` — Skip automatic dtype inference for unspecified columns
